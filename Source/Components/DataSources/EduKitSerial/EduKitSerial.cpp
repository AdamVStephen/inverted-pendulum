...
static void SerialThreadFunction(EduKitSerial &serial);

EduKitSerial::EduKitSerial() : DataSourceI() {
    synchSem.Create();
    synchSem.Reset();
...
bool EduKitSerial::Initialise(MARTe::StructuredDataI& data) {
...
bool EduKitSerial::AllocateMemory() {
    return true;
}

bool EduKitSerial::GetSignalMemoryBuffer(const uint32 signalIdx, const uint32 bufferIdx,
                                                 void *&signalAddress) {
    if (signalIdx == 0) {
...
const MARTe::char8 *EduKitSerial::GetBrokerName(StructuredDataI &data,
...
bool EduKitSerial::GetInputBrokers(ReferenceContainer &inputBrokers,
...
bool EduKitSerial::GetOutputBrokers(ReferenceContainer &outputBrokers,
                                            const char8* const functionName,
                                            void * const gamMemPtr) {
    return false;
}

bool EduKitSerial::SetConfiguredDatabase(StructuredDataI & data) {
...
bool EduKitSerial::PrepareNextState(const char8 * const currentStateName,
                                    const char8 * const nextStateName) {
    return true;
}

CLASS_REGISTER(EduKitSerial, "1.0");
